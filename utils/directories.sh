...
for D in /etc /root /boot /var/log /var/cache/farm /var/cache/bind; do
	echo $D
done
...
for D in `ls -F /var/lib |grep / |egrep -v "^(AccountsService|alsa|alternatives|apparmor|apt|apt-xapian-index|aptitude|belocs|aspell|binfmts|clamav|colord|cron-apt|dictionaries-common|dkms|doc-base|emacsen-common|gconf|gems|ghc|ghostscript|haproxy|initramfs-tools|iptotal|iptraf|ispell|libreoffice|libxml-sax-perl|lightdm|lightdm-data|locales|logcheck|logrotate|lxc|lxcfs|lxd|lxdm|man-db|mdadm|mibs|mlocate|monit|mplayer|mysql|nfs|nginx|ntp|ntpdate|openbox|plymouth|polkit-1|postfix|postgresql|python|python-support|quota|resolvconf|rfkill|scrollkeeper|sepolgen|setroubleshoot|sgml-base|smartmontools|snapd|snmp|stateless|sudo|systemd|tex-common|texmf|ubiquity|ubuntu-drivers-common|ubuntu-release-upgrader|udisks2|update-manager|update-notifier|update-rc.d|urandom|upower|ureadahead|usbutils|VBoxGuestAdditions|vim|xfonts|xkb|xml-core)/$" |cut -d/ -f1`; do
	echo /var/lib/$D
done
